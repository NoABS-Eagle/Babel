...
def offset_contour(vertices, r: float, inward: bool = False) -> list[Edge]:
    ...
    `inward=True` сдвигает ВНУТРЬ на те же `r` — обратная задача, нужная
    импортёру: KiCad хранит у зоны край меди, а осевая пера лежит внутри
    него на полпера. Отдельного алгоритма это не требует: сдвиг внутрь —
    это сдвиг наружу для контура противоположного обхода, поэтому
    меняется только то, к какой ориентации контур нормализуется. Углы
    меняются ролями сами собой: где наружу росла дуга, внутрь идёт
    подрезка, и наоборот.
    ...
    if (contour_area(vertices) > 0) if inward else (contour_area(vertices) < 0):
        n = len(vertices)
